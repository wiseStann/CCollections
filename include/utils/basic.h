...
#define _INDEX_ERROR(index) \
  panic("%s:%d: in '%s': given index %d is not within the bounds of container", __FILE__, __LINE__, __FUNCTION__, (int)index)
#define _VALUE_ERROR \
  panic("%s:%d: in '%s': given value is not in container", __FILE__, __LINE__, __FUNCTION__)
