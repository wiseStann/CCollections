...
> Given array must be not empty.
> Given array must consist of digits.
> Complex time - O(n).

 Parameters [in]:
    -> [array], an array, which sum should be found

 Parameters [out]:
    -> [sum], the sum of all elements of a given array
...
double arraySum(Array* array)
{
    ...
    double sum;
    for (int i = 0; i < array->size; i++) {
        sum += (double)(size_t)array->buff[i];
    }
    return sum;
}
...
Getting the average value of array.
> Given array must be not empty.
> Given array must consist of digits.
> Complex time - O(n).

 Parameters [in]:
    -> [array], an array, which average should be found

 Parameters [out]:
    -> [average_val], an average value of array elements
...
double arrayMean(Array* array)
{
    ...
    double arr_sum = arraySum(array);
    double average_val = arr_sum / array->size;
    return average_val;
}
...
> Complex time - O(n).
 ...
 Parameters [in]:
    -> [array], an array, the minimum of which should be found

 Parameters [out]:
    -> [min_el], the minimum element, which should be found
...
void* arrayMin(Array* array)
{
    ...
    void* min_el = array->buff[0];
    for (int i = 1; i < array->size; i++) {
        if (array->buff[i] < min_el) {
            min_el = array->buff[i];
        }
    }
    return min_el;
...
> Complex time - O(n).
 ...
 Parameters [in]:
    -> [array], an array, the maximum of which should be found

 Parameters [out]:
    -> [max_el], the maximum element, which should be found
...
void* arrayMax(Array* array)
{
    ...
    void* max_el = array->buff[0];
    for (int i = 1; i < array->size; i++) {
        if (array->buff[i] > max_el) {
            max_el = array->buff[i];
        }
    }
    return max_el;
